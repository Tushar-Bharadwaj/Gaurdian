...
Write the exploitation queue to `guardian/scans/<current-scan>/vuln/auth-queue.json`. The file must conform to `guardian-skills/schemas/queue-schema.json`.
...
- Before starting analysis: `bash guardian-skills/scripts/update-state.sh <state-file> vuln-auth in_progress`
